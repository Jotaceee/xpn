 ...
 */
 ...
 /* ... Global variables / Variables globales ........................ */
   ...
   char *hsm_arch = "/home/lab/conf/hsm_arch.txt";
 ...
 /* ... Auxiliar functions / Funciones auxiliares ......................................... */
   ...
   /**
    * Check that the path is a partition of XPN
    */
   int is_xpn_partition   ( const char * path )
   {
      return (path[1] == 'x' && path[2] == 'p' && path[3] == 'n');
   }
   ...
   int stat64_to_stat ( struct stat *buf, struct stat64 *st )
   {
    ...
    /*
     * This function initializes the HSM architecture.
     */
    int xpn_hsm_initialize ( void )
    {
      ...
      {
        hsm_arch = hsm_arch_path;
      }

      int fd = open(hsm_arch, O_RDONLY);
      if (fd < 0) return -1;
      ...
      while ((n = read(fd, buf, sizeof(buf))) > 0) {
        for (ssize_t i = 0; i < n; i++) {
          if (buf[i] == '\n') {
            ...
            if (sscanf(line, "%s %s", path, value_str) == 2) {
              tier = realloc(tier, (count + 1) * sizeof(HSM_TIER));
              tier[count].mount_path = strdup(path);
    ...
    int xpn_adaptor_keepInit ( void )
    {
      ...
      if (0 == xpn_adaptor_initCalled)
      {
          ...
          // Initialize HSM architecture
          ret = xpn_hsm_initialize ();
          if (ret < 0)
          {
              debug_error( "ERROR: Expand xpn_hsm_initialize couldn't be initialized :-(\n");
              xpn_adaptor_initCalled = 0;
    ...
    int open ( const char *path, int flags, ... )
    {
      ...
      if (is_xpn_prefix(path))
      {
        ...
        for (i = 0; i < num_tiers; i++) {
          ...
          if (tier[i].is_xpn) {
            if (mode != 0) {
              fd = xpn_open((char *)updated_path, flags, mode);
            }
            else {
              fd = xpn_open((char *)updated_path, flags);
            }
          } else {
              fd = dlsym_open2((char *)updated_path, flags, mode);
          }
    ...
    int statfs64 (const char *path, struct statfs64 *buf)
    {
      ...
      if (is_xpn_prefix(path))
      {
        ...
        for (int i = 0; i < num_tiers; i++) {
          ...
          if (tier[i].is_xpn) {
            ret = xpn_stat(updated_path, (struct stat *) buf);
          } else {
            ret = dlsym_statfs64(updated_path, buf);
          }
      ...
      if (is_xpn_prefix(path))
      {
        ...
        for (int i = 0; i < num_tiers; i++) {
          ...
          if (tier[i].is_xpn) {
            ...
            ret = dlsym_statfs(updated_path, buf);
          }
    ...
    int __lxstat64 ( int ver, const char *path, struct stat64 *buf )
    {
      ...
      if (is_xpn_prefix(path))
      {
        ...
        for (int i = 0; i < num_tiers; i++) {
          ...
          if (tier[i].is_xpn) {
            ret = xpn_stat(updated_path, &st);
            if (ret >= 0) {
              stat_to_stat64(buf, &st);
            }
          } else {
            ret = dlsym_lxstat64(ver, (const char *)updated_path, buf);
          }
      ...
      if (is_xpn_prefix( path))
      {
        ...
        for (int i = 0; i < num_tiers; i++) {
          ...
          if (tier[i].is_xpn) {
            ...
            ret = dlsym_xstat64(ver,(const char *)updated_path, buf);
          }
      ...
      if (is_xpn_prefix(path))
      {
        ...
        for (int i = 0; i < num_tiers; i++) {
          ...
          if (tier[i].is_xpn) {
            ...
            ret = dlsym_lstat(ver,(const char *)updated_path, buf);
          }
      ...
      if (is_xpn_prefix(path))
      {
        ...
        for (int i = 0; i < num_tiers; i++) {
          ...
          if (tier[i].is_xpn) {
            ...
            ret = dlsym_stat(ver,(const char *)updated_path, buf);
          }
    ...
    int rename ( const char *old_path, const char *new_path )
    {
      ...
      if (is_xpn_prefix( old_path) && is_xpn_prefix( new_path))
      {
        ...
        for (int i = 0; i < num_tiers; i++) {
          ...
          char updated_old_path[512], updated_new_path[512];
          snprintf(updated_old_path, sizeof(updated_old_path), "%s/%s", tier[i].mount_path, skip_xpn_prefix(old_path));
          snprintf(updated_new_path, sizeof(updated_new_path), "%s/%s", tier[i].mount_path, skip_xpn_prefix(new_path));

          if (tier[i].is_xpn) {
            ret = xpn_rename(updated_old_path, updated_new_path);
          } else {
            ret = dlsym_rename(updated_old_path, updated_new_path);
          }
      ...
      if (is_xpn_prefix(path))
      {
        ...
        for (i = 0; i < num_tiers; i++) {
          ...
          if (tier[i].is_xpn) {
            ...
            {
              case 'r':
                  fd=xpn_open(updated_path, O_RDONLY | O_CREAT, 0640);
                  break;
              case 'w':
                  fd=xpn_open(updated_path, O_WRONLY | O_CREAT | O_TRUNC, 0640);
                  break;
              default:
                  fd=xpn_open(updated_path, O_RDWR | O_CREAT | O_TRUNC, 0640);
                  break;
            }
            ...
            ret = dlsym_fopen((const char *)updated_path, mode);
          }
    ...
    int mkdir ( const char *path, mode_t mode )
    {
      ...
      if (is_xpn_prefix(path))
      {
        ...
        for (int i = 0; i < num_tiers; i++) {
          ...
          if (tier[i].is_xpn) {
            ret = xpn_mkdir( updated_path ,mode );
          } else {
            ret = dlsym_mkdir((char *)updated_path,mode);
          }
    ...
    DIR *opendir ( const char *dirname )
    {
      ...
      if (is_xpn_prefix(dirname))
      {
        ...
        for (int i = 0; i < num_tiers; i++) {
          ...
          char updated_path[512];
          snprintf(updated_path, sizeof(updated_path), "%s/%s", tier[i].mount_path, skip_xpn_prefix(dirname));

          if (tier[i].is_xpn) {
            ret = xpn_opendir((const char *)(updated_path));
            if (ret != NULL) {
              fdsdirtable_put ( ret );
            }
          } else {
            ret = dlsym_opendir((char *)updated_path);
          }
      ...
      if (is_xpn_prefix(path))
      {
        ...
        for (int i = 0; i < num_tiers; i++) {
          ...
          if (tier[i].is_xpn) {
            ...
            ret = dlsym_rmdir((char *)updated_path);
          }
    ...
    int chdir ( const char *path )
    {
      ...
      if (is_xpn_prefix(path))
      {
        ...
        for (int i = 0; i < num_tiers; i++) {
          ...
          if (tier[i].is_xpn) {
            ret = xpn_chdir((char *)updated_path);
          } else {
            ret = dlsym_chdir((char *)updated_path);
          }
    ...
    int chmod ( const char *path, mode_t mode )
    {
      ...
      if (is_xpn_prefix(path))
      {
        ...
        for (int i = 0; i < num_tiers; i++) {
          ...
          if (tier[i].is_xpn) {
            ret = xpn_chmod(updated_path, mode);
          } else {
            ret = dlsym_chmod((char *)updated_path, mode);
          }
    ...
    int chown ( const char *path, uid_t owner, gid_t group )
    {
      ...
      if (is_xpn_prefix(path))
      {
        ...
        for (int i = 0; i < num_tiers; i++) {
          ...
          if (tier[i].is_xpn) {
            ret = xpn_chown(updated_path, owner, group);
          } else {
            ret = dlsym_chown((char *)updated_path, owner, group);
          }
